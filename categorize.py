...
if __name__ == "__main__" : # python way to declare "main" function
  ...
  if use_of_gathered_model:
    docModel = pickle.load(open("model_gathered.pkl", 'rb'))
...
with open(devfilename, "r", encoding="utf-8") as fic, open(truename + "-pred-knearest" + suff_supplement + ".txt", "w", encoding="utf-8") as fic_dest:
  for file in fic :
