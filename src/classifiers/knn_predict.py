...
if __name__ == "__main__" : # python way to declare "main" function
  ...
  truename = devfilename[:-4].split("/")[2]
  suff_supplement = ""
  for particle in modelName[1:] : 
    suff_supplement += "-" + particle
with open(devfilename, "r", encoding="utf-8") as fic, open("results/knn/" + truename + "-pred-knearest" + suff_supplement + ".txt", "w", encoding="utf-8") as fic_dest:
  for file in fic :
